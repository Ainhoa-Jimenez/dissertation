...
## 7. Create Chosen_yield column in the plots that require a change in the yield: summer_20_21 X3, summer_22_23 X3, summer_22_23 X4

summer_20_21 X3
```{r}
summer_20_21 <- summer_20_21 %>% 
  mutate(X3.chosen_yield = case_when(
    Day_cum < 44165 ~ X3.Y,
    Day_cum >= 44165 & Day_cum <= 44173 ~ X3.calc_yield,
    Day_cum > 44173 ~ X3.Y))

summer_22_23 <- summer_22_23 %>% 
  mutate(X3.chosen_yield = case_when(
    Day_cum < 44884 ~ X3.Y,
    Day_cum >= 44884 & Day_cum <= 44941 ~ X3.Y_calc,
    Day_cum > 44941 ~ X3.Y),
    X4.chosen_yield = case_when(
    Day_cum < 44898 ~ 0,
    Day_cum >= 44173 ~ X4.Y))
```


## 8. Make plots for PAR and Temperature in each summer, in each sample (check activity under snowcover). Different axis (use for characterising)
...
(combined_plot_22_23_X1 <- (Yield_plot_22_23_X1/temp_plot_22_23_X1/PAR_plot_22_23_X1) & plot_layout(ncol = 1))

ggsave(filename = "graphs_characterise/combined_plot_22_23_X1.png", combined_plot_22_23_X1, device = "png")  # Save graph
```
...
## 8. Make plots for PAR and Temperature in each summer, in each sample (check activity under snowcover). Same axis (use for comparison)
## 9. Plot data for the heatwave March 2022
