...
(temp_plot_march_22_23_X1 <- data_clean %>% 
   filter(Day_cum > 44620 & Day_cum < 44650) %>% # Filter days from 28th Feb 2022
...
```{r}
(combined_plot_19_23 <- ( (yield_year_round_19_23 + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.title.x = element_blank()))/ # use ggarrange to move legend 
  ...
  (PAR_year_round_19_23 + theme(axis.text.x = element_text(angle = 45, hjust = 1))) & plot_layout(ncol = 1)))
...
ggsave(filename = "graphs_year_round/combined_plot_19_23.png", combined_plot_19_23, width = 58,height = 30, units = "cm",device = "png")  # Save graph
```
...
(graph_temp_y_all_period_X3 <-  rbind(
  data_clean %>% 
    select(X3.Temp, X3.chosen_yield),
  data_clean_23 %>% 
    select(X3.Temp, X3.chosen_yield))%>%
  ggplot() +
...
ggsave(filename = "graphs_characterise/austroplaca/graph_temp_y_all_period_X3.png", graph_temp_y_all_period_X3, width = 20,height = 20, units = "cm",device = "png")  # Save graph
...
ggsave(filename = "graphs_characterise/austroplaca/graph_rh_y_all_period_X3.png", graph_rh_y_all_period_X3, width = 20,height = 20, units = "cm",device = "png")  # Save graph
```
12.1.3 PAR- yield
```{r}

(graph_temp_y_all_period_X3 <- data_clean %>% 
  ggplot() +
  geom_point(aes(X3.PAR, X3.chosen_yield, color = "Austroplaca soropelta")) +
  theme_graphs_env_cond()+
  labs(x = "PAR \n(µmol m-2 s^-1)", y = "Yield Lichen X3 ") +
   scale_color_manual(values = c( "Austroplaca soropelta" = "#ff914d")) +
  theme(axis.text.x = element_text(angle = 0),
        legend.position = "none"))
...
```{r}
(graph_temp_y_all_period_moss <- rbind(
  data_clean %>% 
    select(av_moss_yield, av_moss_temp),
  data_clean_23 %>% 
    select(av_moss_yield, av_moss_temp))%>%
...
ggsave(filename = "graphs_characterise/bryum/graph_temp_y_all_period_moss.png", graph_temp_y_all_period_moss, width = 20,height = 20, units = "cm",device = "png")  # Save graph
...
ggsave(filename = "graphs_characterise/bryum/graph_rh_y_all_period_moss.png", graph_rh_y_all_period_moss, width = 20,height = 20, units = "cm",device = "png")  # Save graph
```

12.3. Combined graph

```{r}
graph_temp_y_all_period_X3 <- graph_temp_y_all_period_X3 + theme(axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank())
graph_rh_y_all_period_X3 <- graph_rh_y_all_period_X3 + theme(axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(),  axis.title.y = element_blank())
graph_temp_y_all_period_moss <- graph_temp_y_all_period_moss
graph_rh_y_all_period_moss <- graph_rh_y_all_period_moss + theme(axis.title.y = element_blank())


(combined_temp_yield_rh <- (graph_temp_y_all_period_X3 /graph_rh_y_all_period_X3/graph_temp_y_all_period_moss/graph_rh_y_all_period_moss
                            ) + plot_layout(ncol = 2, nrow = 2))

ggsave(filename = "graphs_env_conditions/combined_temp_yield_rh.png", combined_temp_yield_rh, width = 20,height = 15, units = "cm",device = "png")  # Save graph
...
(PAR_plot_21_22_results <- summer_21_22 %>% 
   ...
   geom_segment(aes(x = as.POSIXct("2021-12-29 00:53:00"), y = 700,
                    xend = as.POSIXct("2021-12-31 00:53:00"), yend = 100), arrow = arrow(length = unit(0.5, "cm")), 
                size = 1, color = "brown") + # add arrow for snowmelting event
    
   geom_segment(aes(x = as.POSIXct("2022-02-18 00:53:00"), y = 700,
                    xend = as.POSIXct("2022-02-16 00:53:00"), yend = 100), arrow = arrow(length = unit(0.5, "cm")), 
                size = 1, color = "darkblue") + # add arrow for short snowfall event
...
(combined_plot_21_22_results <- (ETR_plot_21_22_results/
                                Yield_plot_21_22_results/
                                temp_plot_21_22_results/
                                (PAR_plot_21_22_results + 
                                theme(axis.text.x = element_text(angle = 45, hjust = 1))) 
                                & plot_layout(ncol = 1))) 

ggsave(filename = "graphs_characterise/combined_plot_21_22_results.png", combined_plot_21_22_results, width = 54,height = 18, units = "cm", device = "png")  # Save graph
...
(combined_plot_21_22_undersnow_31_dec <- (ETR_plot_21_22_undersnow_31_dec
                                         /Yield_plot_21_22_undersnow_31_dec
                                         /temp_plot_21_22_undersnow_31_dec
                                         /PAR_plot_21_22_undersnow_31_dec) &
                                         plot_layout(ncol = 1))
...
(combined_plot_21_22_undersnow_16_feb <- (ETR_plot_21_22_undersnow_16_feb
                                         /Yield_plot_21_22_undersnow_16_feb
                                         /temp_plot_21_22_undersnow_16_feb
                                         /PAR_plot_21_22_undersnow_16_feb) 
                                         & plot_layout(ncol = 1))
...
```
### 14.3.Graph of daily pattern of PAR

```{r}
# Close up with time

(PAR_closeup_22_23 <- summer_22_23 %>% 
   mutate(Av_par =rowMeans(select(., c("X1.PAR", "X2.PAR", "X3.PAR", "X4.PAR")), na.rm = TRUE)) %>%  # calculate mean PAR across all samples) %>% 
    filter(Day_cum > 44905 & Day_cum < 44909) %>% # filter days before the 10th of December (when the difference isn't clear)
   ggplot(aes(DateTime, Av_par)) +
   geom_line()+  # Add a line that joins the points
   geom_vline(xintercept = as.numeric(seq(ymd_hms("2022-12-10 00:00:00"), ymd_hms("2023-12-31 00:00:00"), by = "1 day")), color = "black") +
   labs(x = "\nDate (day)", y = "Average PAR\n(µmol m-2 s^-1)\n ") +
   theme_graphs_env_cond() +
   theme(axis.text = element_text(size = 15),  # Adjust the size as needed
          axis.title = element_text(size = 18)) +
      scale_x_datetime(breaks = "2 hour", labels = waiver()) +  # make the scale every hour
   theme(axis.text.x = element_text(angle = 45, hjust = 1)))

ggsave(filename = "graphs_year_round/PAR_closeup_22_23.png", PAR_closeup_22_23, width = 40,height = 15, units = "cm",device = "png")  # Save graph

(PAR_closeup_19_20 <- summer_19_20 %>% 
   mutate(Av_par =rowMeans(select(., c("X1.PAR", "X2.PAR", "X3.PAR", "X4.PAR")), na.rm = TRUE)) %>%  # calculate mean PAR across all samples) %>% 
    filter(No. > 7860 & No. < 7940) %>% # filter days before the 10th of December (when the difference isn't clear)
   ggplot(aes(Date, Av_par)) +
   geom_line()+  # Add a line that joins the points
   geom_vline(xintercept = as.numeric(seq(ymd_hms("2019-12-10 00:00:00"), ymd_hms("2019-12-31 00:00:00"), by = "1 day")), color = "black") +
   labs(x = "\nDate (day)", y = "Average PAR\n(µmol m-2 s^-1) ") +
   theme_graphs_env_cond() +
      scale_x_datetime(breaks = "1 hour", labels = waiver()) +  # make the scale every hour
   theme(axis.text.x = element_text(angle = 45, hjust = 1)))
```

### 14.4 Plot heatwave march
...
## 15. Further code results

1. Test minths in which air T > 0
```{r}
...
2. Check when was the min temp for summer 22-23 recorded
