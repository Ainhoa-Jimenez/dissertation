...
1. Generate a big dataset with all the data

```{r}
data_comb_until_2022 <- data_clean[]
```
...
total_activetime_stats_X3 <- bind_rows(  # put both counts into one dataset
  data_clean %>% count(Condition = X3.Y > 0, name = "Count_data_clean"),  # OCunt rows with active time (1 row = 1 hour)
  data_clean_23 %>% count(Condition = X3.Y > 0, name = "Count_data_clean_23")) %>% 
group_by(Condition) %>% 
    mutate_all(~ifelse(is.na(.), 0, .)) %>% 
 summarise_all(sum) %>%  # join rows that have the same condition
mutate(Total_time = Count_data_clean + Count_data_clean_23) # add both columns to get the total active time of the year
  
(total_active_hours <- activetime_stats_X3 %>% # filter form thw dataset the total number of active hours
...
summer_activetime_stats_X3 <-bind_rows(
  summer_19_20 %>% count(Condition = X3.Y > 0, name = "Count_19_20"),
  summer_20_21 %>% count(Condition = X3.Y > 0, name = "Count_20_21"),
summer_21_22 %>% count(Condition = X3.Y > 0, name = "Count_21_22"),
summer_22_23 %>% count(Condition = X3.Y > 0, name = "Count_22_23")) %>% 
group_by(Condition) %>% 
    mutate_all(~ifelse(is.na(.), 0, .)) %>% 
 summarise_all(sum) 

(total_active_hours_19_20 <- summer_activetime_stats_X3 %>%
  filter(Condition == TRUE) %>%
  select(Count_19_20))
...
(Percentage_active_hours_19_20 <- total_active_hours_19_20 /nrow(summer_19_20) * 100)
