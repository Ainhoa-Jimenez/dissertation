...
(yield_year_round_19_23 <- rbind(
  ...
  geom_line(aes(Date, X4.chosen_yield, color = "Bryum argenteum"), alpha = 0.3) +
...
data_met %>% 
  ...
  scale_color_manual(values = c("Bryum argenteum" = "#119683", "Austroplaca soropelta" = "#ff914d", "Air Temperature" = "black")) +
...
(Yield_plot_19_20_X3_talk <- summer_19_20 %>% 
   ggplot() +
   geom_line(aes(Date, X4.Y, color = "Bryum argenteum"), alpha = 1)+
...
(Yield_plot_22_23_X3_talk <- summer_22_23 %>% 
   ggplot() +
   geom_line(aes(Date, X4.chosen_yield, color = "Bryum argenteum"), alpha = 1)+
...
ggsave(filename = "graphs_talk/combine_plot_yield_aus.png", combine_plot_yield_aus, width = 24,height = 12, units = "cm", device = "png")  # Save graph
```
