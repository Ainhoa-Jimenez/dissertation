...
```{r}
data_corr_temp <- read.csv("correlation_temp.csv")
...
TT over time
```{r}
data_temp_summary <- data_corr_temp %>% 
  mutate(year = as.factor(year),
         sample = as.factor(sample),
         species = if_else(sample == "X3", "Austroplaca soropelta", "Bryum argenteum")) %>% 
  group_by(species, year) %>% 
  summarise(mean_temp = mean(av_temp_act),
            sd = sd(act_hours))



(graph_temp <- data_temp_summary %>%
  ggplot(aes(x = year, y = mean_temp, color = species, group = species)) +
  geom_point(size = 3) +
    geom_errorbar(aes(ymin = mean_temp - sd, ymax = mean_temp + sd), width = 0.2)+
  scale_color_manual(values = c("Bryum argenteum" = "#119683", "Austroplaca soropelta" = "#ff914d")) +
 theme_graphs_env_cond()+
  labs(x = "\nSummer", y = "TT (ºC") +
  theme_graphs_env_cond() +
...
ggsave(filename = "graphs_correlation/graph_TT.png", graph_decline, width = 16, height = 10, units = "cm", device = "png")  # Save graph
...
```{r}
data_Y_summary <- data_corr_temp %>% 
  mutate(year = as.factor(year),
         sample = as.factor(sample),
         species = if_else(sample == "X3", "Austroplaca soropelta", "Bryum argenteum")) %>% 
  group_by(species, year) %>% 
  summarise(mean_Y = mean(av_yield),
            mean_max_Y = mean (mx_yield),
            sd_mean = sd(av_yield),
            sd_max = sd(mx_yield))

data_Y_summary_X3 <-  data_corr_temp %>% 
  mutate(year = as.factor(year),
         sample = as.factor(sample),
         species = if_else(sample == "X3", "Austroplaca soropelta", "Bryum argenteum")) %>% 
  filter(species == "Austroplaca soropelta") 

kruskal.test(av_yield ~ year, data = data_Y_summary_X3)
# teh results are significantly different. Now less check

(graph_av_yield <- data_Y_summary %>%
  ggplot(aes(x = year, y = mean_Y, color = species, group = species)) +
  geom_point(size = 3) +
    geom_errorbar(aes(ymin = mean_Y - sd_mean, ymax = mean_Y + sd_mean), width = 0.2)+
  scale_color_manual(values = c("Bryum argenteum" = "#119683", "Austroplaca soropelta" = "#ff914d")) +
 theme_graphs_env_cond()+
  labs(x = "\nSummer", y = " Average \nYield") +
  theme_graphs_env_cond() +
...
ggsave(filename = "graphs_correlation/graph_av_yield.png", graph_av_yield, width = 16, height = 10, units = "cm", device = "png")  # Save graph

(graph_max_yield <- data_Y_summary %>%
  ggplot(aes(x = year, y = mean_max_Y, color = species, group = species)) +
  geom_point(size = 3) +
    geom_errorbar(aes(ymin = mean_max_Y - sd_max, ymax = mean_max_Y + sd_max), width = 0.2)+
  scale_color_manual(values = c("Bryum argenteum" = "#119683", "Austroplaca soropelta" = "#ff914d")) +
 theme_graphs_env_cond()+
  labs(x = "\nSummer", y = " Average \nYield") +
  theme_graphs_env_cond() +
